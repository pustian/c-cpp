...
int replace_pattern_char(char *replaced_str, const char *zh_str, const char *pattern, const char repace_char) {
    regex_t reg;
...
int main() {
    ...
    char buf[1024] = {0};
    replace_pattern_char(buf, str0, pattern, '+');
    printf("str0 len=%d val=%s\n buf len=%d val=%s \n",strlen(str0), str0, strlen(buf),buf);
    memset(buf, 0x0, 1024);
    replace_pattern_char(buf, str1, "[ |a-z]+", '*');
