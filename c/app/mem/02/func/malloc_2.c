#include <string.h>
#include "malloc_2.h"
// gcc -DTEST -DMONITOR malloc_2.c -g
...
// 通过日志文件可以监控使用情况，甚至使用%
#ifdef MONITOR 
static void monitor(malloc_head_t *ptr_head) {
...
int init_malloc(malloc_head_t **ptr_ptr_head) {
    ...
    malloc_head_t *ptr_head = malloc(sizeof(malloc_head_t *));;
    void *ptr =  malloc(head_size);
    if(NULL == ptr ) {
        ptr_head->p_size = 0;
        ptr_head->ptr_malloc = NULL;
        return -1;
    }
    memset((void *)ptr, 0x0,  head_size);
    ...
    // for(i = 0; i< _MAX_PATCH_SIZE; ++i) {
    //      ptr_head->ptr_malloc[i].data = NULL;
...
void destroy_malloc(malloc_head_t **ptr_ptr_head) {
    ...
    if(ptr_ptr_head != NULL) {
        malloc_head_t *ptr_head = *ptr_ptr_head;
        int i=0; 
        for(i=0; i<_MAX_PATCH_SIZE; ++i) {
            printf("ptr_head->ptr_malloc[i].data=%p",  ptr_head->ptr_malloc[i].data);
            if(0 != ptr_head->ptr_malloc[i].used_bytes && NULL != ptr_head->ptr_malloc[i].data) {
                free(ptr_head->ptr_malloc[i].data);
                ptr_head->ptr_malloc[i].used_bytes = 0;
            }
        }
        if(ptr_head->ptr_malloc != NULL)
...
void *p_malloc(malloc_head_t *ptr_head, size_t num) {
    ...
    if(num > 0 && num <= _DEFAULT_BLOCK_SIZE) {
        do {
            malloc_t *ptr_malloc = &(ptr_head->ptr_malloc[i]);
            if(NULL != ptr_malloc && 0 == ptr_malloc->used_bytes && NULL == ptr_malloc->data) {
                ptr = malloc(num);
                ptr_malloc->data = ptr;
                ptr_malloc->used_bytes = num;
            }
...
int p_free(malloc_head_t *ptr_head, const void *ptr) {
    ...
    int i = 0;
    for(i=0; i<_MAX_PATCH_SIZE; ++i){
        if(ptr == ptr_head->ptr_malloc[i].data && NULL != ptr ) {
            ptr_head->ptr_malloc[i].used_bytes = 0;
            free(ptr_head->ptr_malloc[i].data);
            break;
        }
